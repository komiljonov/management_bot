...
<<<<<<< HEAD
                ...
                CONFIRMED_REQUESTS: [MessageHandler(Filters.regex("^kelgan so'rovlar$"), self.confirmed_come_requests), MessageHandler(
                    Filters.regex("^yuborilgan so'rovlar$"), self.confirmed_sent_requests), MessageHandler(Filters.regex("ortga$"), lambda update, context: self.start(update, context))],
                DENIED_REQUESTS: [MessageHandler(Filters.regex("^kelgan so'rovlar$"), self.denied_come_requests), MessageHandler(
                    Filters.regex("^yuborilgan so'rovlar$"), self.denied_sent_requests), MessageHandler(Filters.regex("ortga$"), lambda update, context: self.start(update, context))]
=======
...
>>>>>>> b184619415679078d0b887957bb046181049a2aa
            },
            fallbacks=[
    ...
    def accept_request_from_user(self, update: Update, context: CallbackContext):
        ...
        if req['status'] == 0:
                ...
                if update_status['ok']:
                    for msg in sent_msgs2:
                        try:
                            ...
                            context.bot.edit_message_reply_markup(chat_id=msg[3], message_id=msg[2])
                        except:
                            pass
                        try:
                            context.bot.send_message(text=text, chat_id=msg[3], parse_mode="HTML")
                        except: pass
                    try:
                        print(req['user']['chat_id'])
                        context.bot.send_message(
                            text=text, chat_id=req['user']['chat_id'], parse_mode=ParseMode.HTML).pin()
                    except:
                         pass
            else:
            ...
            update.callback_query.answer(
                "Kechirasiz bu so'rov tasdiqlanib bo'lgan!" if req['status'] == 1 else "Kechirasiz bu so'rov rad etib bo'lingan!", show_alert=True)
            return MENU
            ...
            if update.callback_query.message.chat.type == "private":
                ...
                if is_confirmer(req, user):
                    ...
                    context.user_data['checking_request'] = int(
                        update.callback_query.data.split(":")[1])
                    update.callback_query.message.reply_text(
                        "So'rovdagi kamchiliklarni yozing!")
                    return GET_COMMENT_FOR_REQUEST
                else:
                    update.callback_query.answer(
                        "Kechirasiz siz bu so'rov tasdiqlovchilar qatorida emassiz!", show_alert=True)
