...
class _Authentication:
    ...
    def number(self, update:Update, context:CallbackContext):
        ...
        context.user_data['request_register_number'].delete()
        context.user_data['request_register_description'] = update.message.reply_text("Iltimos endi o'zingiz haqingizdagi malumotlarni text ko'rinishida yuboring!\nAdmin bu malumotlarni o'qiydi va sizga bo'tga kirish uchun ruxsat beradi!", reply_markup=ReplyKeyboardRemove())
        return DESCRIPTION
    ...
    def description(self, update:Update, context:CallbackContext):
        ...
        context.user_data['request_register_description'].delete()
    ...
    def wait_start(self, update:Update, context:CallbackContext):
        ...
        if res['status'] == 0:
            update.message.reply_text(f"Sizning so'rovingiz kutilmoqda!", reply_markup=ReplyKeyboardRemove())
        elif res['status'] == 2:
            update.message.reply_text(f"Sizning so'rovingiz bekor qilingan!", reply_markup=ReplyKeyboardRemove())
        elif res['status'] == 1:
            ...
            context.user_data['greeting'] = update.message.reply_text("Kechirasiz sizning so'rovingiz topilmadi!\nIltimos qayta ro'yxatdan o'ting!\nIsmingizni yuboring!", reply_markup=ReplyKeyboardRemove())
            return NAME
